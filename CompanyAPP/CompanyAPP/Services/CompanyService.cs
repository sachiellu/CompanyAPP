...
namespace CompanyAPP.Services
{
    public class CompanyService : ICompanyService
    {
        ...
        {
            // 1. 先建立查詢物件 (這裡用 AsQueryable )
            var companies = _context.Company.AsQueryable();

            // 2. 如果有搜尋字串，就加上 Where 條件
            if (!string.IsNullOrEmpty(searchString))
            {
                companies = companies.Where(c =>
                    c.Name.Contains(searchString)           // 搜尋名稱
              //      c.Industry.Contains(searchString) ||  // 搜尋產業
              //      c.TaxId.Contains(searchString) ||     // 搜尋統編
              //      c.Id.ToString() == searchString       // 搜尋 ID
                );
            }
            // 3. 最後執行查詢並回傳 List
