    ...
    using CompanyAPP.Dtos.Employees;
    ...
    namespace CompanyAPP.Controllers.Api
    {
        ...
        public class EmployeesApiController : ControllerBase
        {
            ...
            public async Task<IActionResult> GetEmployees([FromQuery] string? searchString)
            {
                ...
                var result = emps.Select(e => new EmployeeListDto
                {
                    Id = e.Id,
                    StaffId = e.StaffId,
                    Name = e.Name,
                    Position = e.Position,
                    Email = e.Email,
                    Status = (int)e.Status, // 將 Enum 轉為數字
            ...
            public async Task<IActionResult> CreateEmployee([FromBody] EmployeeUpdateDto dto)
            {
            ...
            [HttpPut("{id}")]
            [Authorize(Roles = "Admin,Manager")]
            public async Task<IActionResult> UpdateEmployee(int id, [FromBody] EmployeeUpdateDto dto) // 修正這裡
            {
                var emp = await _employeeService.GetByIdAsync(id);
                if (emp == null) return NotFound(new { message = "找不到員工" });
            ...
            public async Task<IActionResult> SendInvite(int id)
            {
                ...
                Console.WriteLine($" 呼叫 EmailSender 寄信給: {emp.Email}");
