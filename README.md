...
**架構遷移中 (v3.0 Architecture Transition)**

> 本專案正從傳統的 **ASP.NET Core MVC** 遷移至 **React (Vite) + Web API** 的前後端分離架構。

> - **穩定版本 (Stable)**: 請切換至 `main` 分支。該版本具備完整的 CI/CD 流程、OWASP ZAP 資安防護。

> - **現代化開發分支 (Dev)**: 即為本 `feature/react-modernization` 分支，目前正進行員工模組與廠商模組的 React 移植。

---

## 新版啟動方式：

1. **後端 API**: 進入 `/CompanyAPP` 執行 `dotnet run`

2. **前端 React**: 進入 `/company-frontend` 執行 `npm run dev`
