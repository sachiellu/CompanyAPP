    ...
    return (
        <div className="page-container position-relative px-4 pt-3 text-start">
            <div className="page-header-wrapper mb-3">
                <h2 className="text-dark fw-bold mb-0" style={{ fontSize: '1.5rem' }}>系統概覽</h2>
                <div className="text-muted small">登入帳號: {userEmail}</div>
            </div>

            <div className="row g-3 mb-4">
                {[
                    { label: '廠商總數', value: stats.totalCompanies, icon: 'bi-building', color: 'primary', to: '/companies' },
                    { label: '在職員工', value: stats.totalEmployees, icon: 'bi-people', color: 'success', to: '/employees' },
                    { label: '待處理任務', value: stats.pendingMissions, icon: 'bi-clipboard-check', color: 'warning', to: '/missions' }
                ].map((card, i) => (
                    <div key={i} className="col-md-4">
                        <Link to={card.to} className="text-decoration-none text-dark">
                            <div className="card shadow-sm border-0 p-3 hover-bg-light transition-all">
                                <div className="d-flex align-items-center">
                                    <div className={`bg-${card.color}-subtle text-${card.color} p-2 rounded-3 me-3`}><i className={`bi ${card.icon} fs-4`}></i></div>
                                    <div><div className="text-muted small fw-bold">{card.label}</div><div className="fs-4 fw-bold">{card.value}</div></div>
                                </div>
                            </div>
                        </Link>
                    </div>
